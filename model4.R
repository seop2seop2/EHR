...
source("model4_function.R")
source("making_simulation_sample.R")

fileNameRoot = "model4-" 
numSavedSteps=15000 ; thinSteps=2

myData<-read.table("simul_data.txt", sep="@")
...
times_number<-c(1,1.04,1.1,1.16,1.25,1.5,2)
...
v.square=1/(n1*p1_data*(1-p1_data))
u.square=1/(n2*p2_data*(1-p2_data))

model4_data=data.frame(phi_data=c(phi1_data,phi2_data), v=v.square, 
                       u=u.square,s=1:30,delta1=c(rep(1,30),rep(0,30)),delta2=c(rep(0,30),rep(1,30)))
sigmahat=mlest(cbind(phi1_data,phi2_data))$sigmahat
  #.............................................................................
  #graphFileType = "eps" 
  #------------------------------------------------------------------------------- 
  ...
  #startTime = proc.time()
  mcmcCoda = genMCMC( data=model4_data , yName=yName , uName=uName, vName=vName, sigmahat=sigmahat, xName=xName,
                      numSavedSteps=numSavedSteps , thinSteps=thinSteps , select_model="model4.txt",
...
save.image(file ="archive_model4.RData")
