...
genMCMC = function( data , xName="x" , yName="y" , sName="s", uName="u", vName="v", sigmahat,
  ...
  modelString = "
   model {
    ...
    for ( sIdx in 1:Nsubj ) {
      phi[sIdx,1:2] ~ dmnorm(mu , sigmahat)
    }
...
smryMCMC = function(  codaSamples , 
  ...
  phi1_hat = apply(mcmcMat[,1:30],1,function(x) sum(w1*sigmoid(x)))
  phi2_hat = apply(mcmcMat[,31:60],1,function(x) sum(w2*sigmoid(x)))
  mcmcMat = cbind(mcmcMat,phi1_hat,phi2_hat)
